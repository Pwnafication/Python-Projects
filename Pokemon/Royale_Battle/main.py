...
def Fight(fightVariable1):
    ...
    print()
    if fightVariable1.obj[0].speed > fightVariable1.obj[1].speed:
...
sectionWinners = [list_of_sections[0].winner.name,list_of_sections[1].winner.name,list_of_sections[2].winner.name,list_of_sections[3].winner.name,list_of_sections[4].winner.name,list_of_sections[5].winner.name]

print()

indexInteger1 = 0
for each in sectionWinners:
    for each in pokemon_data:
        if each["name"]["english"] == sectionWinners[indexInteger1]:
            list_of_sections[indexInteger1].winner.hp = (int(each["base"]["HP"])*5)
            print(f"{list_of_sections[indexInteger1].winner.name} has been restored to {list_of_sections[indexInteger1].winner.hp} HP")
    indexInteger1 += 1
print()
indexInteger2 = 0
for each1 in pokemon_list:
    for each2 in pokemon_data:
        if each2['name']['english'] == each1.name:
            each1.hp = (int(each2['base']['HP'])*5)
